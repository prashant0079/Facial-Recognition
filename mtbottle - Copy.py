...
if __name__ == '__main__':
    ...
    button_clicked = 1
    customroot=''
    #root='/home/phoenix/facialrecognition/
...
#                break
    ...
    def database(names):
        ...
        for name in names:
            ...
            c.close()
            if len(result) < 1:
                conn.execute("INSERT INTO notif (name,time,read) VALUES (?, ?, ?)", (name, datetime.now(),False))
                print("In If len")
                conn.commit()
...
#        return output
    ...
    def load_notification():
        conn = sqlite3.connect('notification.db')
        c = conn.cursor()
        c.execute("SELECT name,time FROM notif ORDER BY time DESC")
        result = c.fetchall()
        print(result)
        print(len(result))
        c.close()
